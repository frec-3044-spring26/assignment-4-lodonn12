...
ggplot(sunapee, aes(x = year, y = ice_off_doy)) +
  ...
  geom_line() + 
  labs(
    title = "Lake Sunapee Ice-Off Date Over Time",
    x = "Year",
...
**Answer 2:**
I expect the slope to be negative. WIth climate warming, lakes tend to thaw earlier in the year. Since earlier dates correspond to smaller day-of-year, ice off DOY should decrease over time. Therefore negative slope.
...
**Answer 3:**
The slope is negative. This matches expectations of question 2 because of warming temps are expected to cause earlier ice off dates over time 
...
**Answer 4:**
R^2 = 0.2055
This is a pretty low R^2 value most likely because year alone does not capture all climate variability
...
ggplot(sunapee, aes(x = resid)) + 
  ...
  theme_minimal()
```
The residuals appear approximately normally distributed and centered around zero. There is some slight skew, but generally it satisfies the assumption for linear regression
...
The p-value for the slope is < 0.001. This indicates a very low probability that the true slope is zero. Therefore gives strong statistical evidence that ice-off-DOY is changing over time
...
Slope = -0.10602,
(7 / |-0.10602|) = 66.025 Years for a 1 week change in ice-off-DOY
...
**Answer 9:**
I am moderately comfortable using this model for trend analysis but not for precise yearly prediction. Although the R^2 is relatively low, the slope is statistically significant and the residuals appear approximately normally distributed and centered around 0. This suggests the model captures meaningful long-term trends, even though year alone does not explain all the variability. 
...
ggplot(sunapee, aes(x = year, y = ice_off_doy)) +
  ...
  labs(
    title = "Lake Sunapee Ice-Off with Regression Line",
    x = "Year",
    y = "Ice-Off Day of Year"
  ) +
  theme_minimal()
...
pred_2040
```
Prediction will be approx 102 DOY

```{r}
lower_q <- quantile(sunapee$resid, 0.05)
upper_q <- quantile(sunapee$resid, 0.95)

lower_2040 <- pred_2040 + lower_q
upper_2040 <- pred_2040 + upper_q
...
```{r}
width <- upper_2040 - lower_2040
width
```
The predicted ice-off day in 2040 is approx DOY 102
The 90% confidence interval is approx 87 to 117 DOY
The width of the interval is about 33 days
...
ggplot(sunapee, aes(x = year, y = ice_off_doy)) +
  geom_point() +
  geom_abline(
    intercept = fit_early$coefficients[1],
    slope = fit_early$coefficients[2],
    color = "red",
    linewidth = 1
  ) +
  geom_abline(
    intercept = fit_late$coefficients[1],
    slope = fit_late$coefficients[2],
    color = "blue",
    linewidth = 1
  ) +
  labs(
    title = "comparison of early vs late period slopes",
    x = "year",
    y = "Ice off DOY"
  ) +
  theme_minimal()
...
The slope for the later period is more negative than the early period, indicating that ice-off dates are changing more rapidly in recent decades. This suggests that warming effects have accelerated over time. Both models are statistically significant which gives further evidence to this conclusion. 
